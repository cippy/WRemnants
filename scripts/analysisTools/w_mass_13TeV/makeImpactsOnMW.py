...
if __name__ == "__main__":
    ...
    parser.add_argument('-x', '--excludeNuisgroups', default=".*eff_(stat|syst)|.*AlphaS$|.*nonClosure|.*resolutionCrctn|.*scaleCrctn|.*scaleClos|.*polVar|.*QCDscale(W|Z)|.*resum.+|.*(muon|ecal)Prefire|FakeRate|theory_ew_|.*pixel|theory$|experiment$|helicity_shower|.*pTModeling|ZmassAndWidth|widthW|sin2thetaZ",   type=str, help='Regular expression for nuisances to be excluded (note that it wins against --keepNuisgroups since evaluated before it')
